...
class JointImageLidarTokenizer(BaseVAE, VideoTokenizerInterface):
    ...
    def encode_lidar(self, state: torch.Tensor) -> torch.Tensor:
        B, C, T, H, W = state.shape
        assert T == 1
        if self.squeeze_for_lidar:
            return self.lidar_vae.encode(state.squeeze(2)).unsqueeze(2)
        return self.lidar_vae.encode(state)
    ...
    def decode_lidar(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        assert T == 1
        if self.squeeze_for_lidar:
            return self.lidar_vae.decode(latent.squeeze(2)).unsqueeze(2)
        return self.lidar_vae.decode(latent)
    ...
    @property
    def spatial_compression_factor(self):
        return self.lidar_vae.spatial_compression_factor
    ...
    @property
    def spatial_resolution(self) -> str:
        return self.lidar_vae.spatial_resolution

    @property
    def pixel_chunk_duration(self) -> int:
        return self.lidar_vae.pixel_chunk_duration

    @property
    def latent_chunk_duration(self) -> int:
        return self.lidar_vae.latent_chunk_duration
